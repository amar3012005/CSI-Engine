<template>
  <div class="health-paper-panel">
    ...
    <!-- In-progress or No Report -->
    <div v-else-if="isReportGenerating" class="paper-body">
      <div class="in-progress">
        <div class="pulse-dot" />
        <span>Medical AI Agent is synthesizing clinical evidence... ({{ taskStatus.progress || 0 }}%)</span>
      </div>
    </div>

    <div v-else-if="!report && !loading" class="paper-body">
      <div class="empty-state">
        <p>No health report available for this simulation.</p>
        <button @click="triggerGeneration" class="gen-btn">Generate Evidence-Based Report</button>
      </div>
    </div>

    <!-- Full paper -->
    <div v-else-if="report" class="paper-body" ref="paperBodyRef">
      <!-- Agent-Generated Content Support -->
      <div v-if="report.markdown_content" class="ieee-paper markdown-body">
        <header class="paper-header">
          ...
          <div v-if="report.metadata?.specialists" class="paper-affiliations">
            <span v-for="spec in report.metadata.specialists" :key="spec" class="affil-line">{{ spec }}</span>
          </div>
        </header>
        <div class="paper-rule" />
        <div v-html="marked(report.markdown_content)"></div>
        <footer class="paper-footer">
          <p>{{ report.disclaimer || 'This report was generated by the HIVEMIND Clinical Simulation Interface. It is for research and training purposes only.' }}</p>
        </footer>
      </div>

      <div v-else class="ieee-paper">
...
const paperTitle = computed(() => {
  ...
  // Support both legacy and agent report structures
  const diffs = report.value.differential_diagnoses || []
  if (diffs.length) return `Evidence-Based Assessment: ${diffs[0].diagnosis}`
  
  // Agent-generated reports might have 'title'
  if (report.value.title) return report.value.title

  const cq = (report.value.case_query || report.value.query || '').replace(/Medical Scenario:.*\n/i, '').trim()
...
const isReportGenerating = computed(() => {
  return ['pending', 'processing', 'generating', 'in_progress'].includes(taskStatus.value?.status)
})

async function fetchReport() {
  try {
    // 1. Try to fetch an existing completed agent-generated health report first
    try {
      const agentRes = await reportApi.getReportBySimulation(props.simulationId, 'health')
      const latestReport = Array.isArray(agentRes.data) ? agentRes.data[0] : agentRes.data
    ...
    // 2. If no completed agent report, check if a generation task is in progress
    try {
      const statusRes = await reportApi.getReportStatus({
        simulation_id: props.simulationId,
        report_type: 'health'
      })

      if (statusRes.data) {
        if (statusRes.data.status === 'completed' && statusRes.data.report_id) {
          const completed = await reportApi.getReport(statusRes.data.report_id)
          report.value = completed.data
          taskStatus.value = null
          stopPolling()
          loading.value = false
          return
        ...
        if (['pending', 'processing', 'generating', 'in_progress'].includes(statusRes.data.status)) {
      ...
      if (![400, 404].includes(statusErr?.response?.status)) {
        throw statusErr
      }
    }

    // 3. Fallback to legacy report fetch if neither above exists
    ...
    console.error('Fetch health report error:', e)
    // Don't set global error yet, maybe retry or wait for task
  }
}
...
async function triggerGeneration() {
  try {
    ...
    taskStatus.value = { status: 'generating', progress: 0 }
    ...
    error.value = "Failed to trigger report generation: " + e.message
    loading.value = false
  }
}
...
<style scoped>
/* Section for Markdown compatibility */
.markdown-body {
  font-size: 12px; line-height: 1.6; color: #333;
}
.markdown-body h1, .markdown-body h2, .markdown-body h3 {
  color: #0a0a0a; margin-top: 1.5em; margin-bottom: 0.5em;
}

.empty-state { text-align: center; padding: 40px 20px; color: #888; }
.gen-btn {
  background: #065f46; color: white; border: none; padding: 10px 20px;
