    ...
    <div class="main-split-layout">
        ...
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              <span class="report-tag">IEEE Paper</span>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <div class="next-step-actions" v-if="isComplete" style="display:flex;gap:10px;">
            <button class="next-step-btn" @click="goToInteraction" style="flex:1;">
            ...
            <button class="next-step-btn secondary" @click="goToPaperReport" style="flex:1;background:transparent;border:1px solid #9CA3AF;color:#4B5563;">
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show special hint when final answer -->
...
const goToPaperReport = () => {
  if (props.simulationId) {
    router.push({ name: 'PaperReport', params: { simulationId: props.simulationId } })
  }
}
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation of reason (exclude summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first question starts with "1.", handle specially
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy to default display if not placeholder
          ...
          // No platform markers (legacy format), use entire text as answer
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbering or only one found, return as-is
      if (matches.length <= 1) {
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholders) with different content
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (only shown when both platforms have real answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 heading since section title is shown in outer wrapper
  ...
  // Clean whitespace after list open tags
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Clean whitespace before list close tags
  ...
  // Process line breaks - empty lines become paragraphs, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <br> between block close and paragraph open tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering: keep incrementing when single-item <ol> blocks are separated by paragraphs
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If complete, scroll to top; otherwise scroll to bottom to follow latest log
            if (isComplete.value) {
